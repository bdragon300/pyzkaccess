...
class TestEvent:
    @pytest.mark.parametrize(
        "event_string", ("2000-02-02 15:09:10,0,7125793,1,27,2,0\r\n", "2000-02-02 15:09:10,0,7125793,1,27,2,0")
    )
    def test_init__should_parse_and_fill_object_attributes(self, event_string):
        ...
        assert obj.door == 1
        assert obj.event_type == 27 and type(obj.event_type) is DocValue
...
class TestEventLog:
    ...
    def test_init__should_initialize_data_attribute(self):
        ...
        assert obj._sdk is self.sdk
        assert obj.buffer_size == 4096
        assert obj.data.maxlen is None  # When _data is passed
        assert obj.only_filters == filters
    ...
    @pytest.mark.parametrize(
        "initial,events_str,expect,expect_len",
        (
            (
                [Event("2000-02-02 15:09:00,0,7125794,2,26,1,0")],
                [
    ...
    def test_refresh__if_events_returned_and_filters_with_maxlen_set__should_save_filtered_data_and_return_count(
        self,
    ):  # noqa
        events_strings = [
            "2000-02-02 15:09:10,0,7125793,1,27,2,0",
    ...
    def test_poll__if_refresh_returned_events__should_return_new_events(self):
        ...
        with patch.object(obj, "refresh", Mock(side_effect=(0, 0, 2))):
            res = obj.poll(60, polling_interval=0.5)

            assert res == list(data)[-2:]

    def test_poll__if_refresh_returned_events_and_filters_specified__should_return_new_filtered_events(self):  # noqa
        data = [
            Event("2000-02-02 15:09:10,0,7125793,1,27,2,0"),
            Event("2000-02-02 15:09:15,0,7125794,3,27,2,0"),
            Event("2000-02-02 15:09:17,0,7125784,1,25,2,0"),
    ...
    def test_poll__if_refresh_returned_events_on_3rd_time__should_hang_for_2_intervals(self):
        ...
        with patch.object(obj, "refresh", Mock(side_effect=(0, 0, 2))):
            start = datetime.now()
            obj.poll(60, polling_interval=0.5)
    ...
    def test_poll__if_refresh_does_not_returned_result__should_return_empty_result_on_timeout(self):
        ...
        with patch.object(obj, "refresh", Mock(wraps=refresh_with_slow_network)):
            start = datetime.now()
            res = obj.poll(2, polling_interval=0.5)
            seconds = (datetime.now() - start).seconds
    ...
    @pytest.mark.parametrize(
        "initial_filters,filters,expect",
        (
            ...
            ({"event_type": {44, 55}}, {"event_type": (33, 44), "door": 1}, {"event_type": {33, 44, 55}, "door": {1}}),
        ),
    )
    def test_only__should_return_instance_with_merged_filters(self, initial_filters, filters, expect):
    ...
    @pytest.mark.parametrize("idx", (slice(None, 2), slice(1, 3), slice(None, None, 2), slice(0, 0)))
    ...
    def test_getitem__if_index_passed_and_filters_specified_and_out_of_range__should_raise_error(self):
        data = deque(
            (
                Event("2000-02-02 15:09:10,0,7125793,1,27,2,0"),
                Event("2000-02-02 15:09:15,0,7125794,3,27,2,0"),
    ...
    @pytest.mark.parametrize("idx", (slice(None, 2), slice(1, 3), slice(None, None, 2), slice(0, 0)))
    def test_getitem__if_slice_passed_and_filters_specified__should_return_filtered_items(self, idx):
        data = deque(
            (
                Event("2000-02-02 15:09:10,0,7125793,1,27,2,0"),
                Event("2000-02-02 15:09:15,0,7125794,3,27,2,0"),
