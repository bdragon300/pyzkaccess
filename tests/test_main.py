...
class TestZKAccess:
    ...
    def test_init__if_no_autoconnect__should_initialize_default_event_log_attributes(self):
        ...
        assert type(obj._event_log) is EventLog
    ...
    def test_download_file__if_buffer_got_overflowed__should_repeat_with_double_buffer_size(self, data_size):
        def se(*a, **kw):
        ...
        obj = ZKAccess(connstr=self.connstr, device_model=ZK400)
        expect_calls = [call("test_file.dat", 1 * 1024 * 1024), call("test_file.dat", 2 * 1024 * 1024)]
    ...
    def test_cancel_alarm__should_call_sdk(self):
        ...
        self.sdk.control_device.assert_called_once_with(ControlOperation.cancel_alarm.value, 0, 0, 0, 0)
    ...
    def test_doors_prop__should_return_object_sequence(self, model, doors_count):
        ...
        assert type(res) is DoorList
        assert all(type(x) is Door for x in res)
    ...
    def test_relays_prop__should_return_object_sequence(self, model, relay_count):
        ...
        assert type(res) is RelayList
        assert all(type(x) is Relay for x in res)
    ...
    def test_readers_prop__should_return_object_sequence(self, model, readers_count):
        ...
        assert all(type(x) is Reader for x in res)
    ...
    def test_aux_inputs_prop__should_return_object_sequence(self, model, aux_input_count):
        ...
        assert type(res) is AuxInputList
        assert all(type(x) is AuxInput for x in res)
    ...
    def test_parameters_prop__should_return_object_of_device_parameters(self):
        ...
        assert type(res) is DeviceParameters
    ...
    def test_device_prop__if_device_was_not_passed_to_init__should_return_new_object(self):
        def se(parameters, buffer_size):
            choices = {"IPAddress": "10.0.0.2", "~SerialNumber": "test serial"}
    ...
    def test_search_devices__should_return_list_of_found_device_objects(self):
        ...
        expect = (
            ...
            ZKDevice(
                mac="00:17:61:C8:EC:18",
                ip="192.168.1.202",
                serial_number="DGD9190019050335135",
                model=ZK200,
                version="AC Ver 4.3.4 Apr 28 2017",
            ),
        )
    ...
    def test_search_devices__if_other_error_raised__should_reraise_it(self):
        ...
        with pytest.raises(ZKSDKError) as e:
            ZKAccess.search_devices("192.168.1.255")
    ...
    def test_connect__if_connected_and_trying_connect_with_another_connstr__should_raise_error(self):
        obj = ZKAccess(connstr=self.connstr)
    ...
    def test_change_ip_address__shoudl_call_sdk_function(self):
        ...
        ZKAccess.change_ip("00:17:61:01:88:27", "192.168.1.100", "255.255.255.0", ChangeIPProtocol.udp, "path")

        self.sdk.modify_ip_address.assert_called_once_with("00:17:61:01:88:27", "192.168.1.100", "255.255.255.0", "UDP")
