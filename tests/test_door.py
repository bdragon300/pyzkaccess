...
class TestDoor:
    ...
    def setup(self):
        ...
        self.obj = Door(
            self.sdk, self.event_log, self.door_number, self.relay_list, self.reader, self.aux_input, self.parameters
        )
    ...
    def test_eq__should_return_comparing_result(self, number):
        other_obj = Door(
            self.sdk, self.event_log, number, self.relay_list, self.reader, self.aux_input, self.parameters
        )
...
class TestDoorList:
    ...
    def setup(self):
        ...
        self.doors = (
            Door(
                ...
                self.event_log,
                1,
                RelayList(self.sdk, (Relay(self.sdk, RelayGroup.lock, 1), Relay(self.sdk, RelayGroup.aux, 1))),
            ...
            Door(
                ...
                self.event_log,
                2,
                RelayList(self.sdk, (Relay(self.sdk, RelayGroup.lock, 2), Relay(self.sdk, RelayGroup.aux, 2))),
    ...
    def test_readers__should_return_reader_objects(self):
        ...
        assert res == (Reader(self.sdk, self.event_log, 1), Reader(self.sdk, self.event_log, 2))

    def test_aux_inputs__should_return_aux_input_objects(self):
        res = self.obj.aux_inputs

        assert type(res) is AuxInputList
        assert res == (AuxInput(self.sdk, self.event_log, 1), AuxInput(self.sdk, self.event_log, 2))

    def test_getitem__if_index_passed__should_return_item(self):
        assert type(self.obj[1]) is Door
